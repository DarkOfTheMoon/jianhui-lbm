...
int output_vtk=1;	
char poreFileName[128]="psi2_LBM_Geometry.vtk";

char outputFileName[128]="sys_cut_geo.vtk";
...
int pore;
	...
	Solid_Int = new int**[nx2*(sym_x+1)];	///*********
	Solid = new int**[nx2];
	for (i=0;i<nx2*(sym_x+1);i++)				///*********
		{
		Solid_Int[i]=new int*[ny2*(sym_y+1)];		///*********
		for (j=0;j<ny2*(sym_y+1);j++)			///*********
			{
			Solid_Int[i][j]= new int[nz2*(sym_z+1)];///*********
	...
	//while (!fin.eof())                                        //**********
		{	
			...
			if ((i>=xn) and (i<xp) and (j>=yn) and (j<yp) and (k>=zn) and (k<zp))
				{Solid[i-xn][j-yn][k-zn]=pore;	if (pore==1) sum++;}
