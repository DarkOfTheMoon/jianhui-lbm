...
{	
	...
	for (int i=1;i<=Count;i++)	
			
		{
			...
			eu=e2[lm][0]*u[i][0]+e2[lm][1]*u[i][1]+e2[lm][2]*u[i][2];
			fg[i][lm]=1.0/6.0*rho_r[i]*(1+3*eu);
			Fg[i][lm]=fg[i][lm];
...
void Backup(int m,double* rho,double* psi, double** u, double** f, double** g)
{
	...
	out.open(name5.str().c_str());
        ...
        out.write((char *)(&g[0][0]), sizeof(double)*(Count+1)*7);    
	out.close();
