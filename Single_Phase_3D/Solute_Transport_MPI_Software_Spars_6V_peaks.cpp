...
void Statistical_psi_general(double**,int, int***);
...
int sol_zf_xp,sol_zf_xn,sol_zf_yp,sol_zf_yn,sol_zf_zp,sol_zf_zn,start_psi;
...
//	fin >> backup_g;                        	fin.getline(dummy, NCHAR);
	...
	fin >> num_psi>>wid_psi>>start_psi;		fin.getline(dummy, NCHAR);
							fin.getline(dummy, NCHAR);
	fin >> decbin;
	...
	MPI_Bcast(&outputfile,128,MPI_CHAR,0,MPI_COMM_WORLD);MPI_Bcast(&num_psi,1,MPI_INT,0,MPI_COMM_WORLD);
	...
	MPI_Bcast(&start_psi,1,MPI_INT,0,MPI_COMM_WORLD);
...
//	reso=reso/Zoom;
	...
	rho_r = new double*[num_psi];
	rho_r[0] = new double[num_psi*(Count+1)];
	for (int i=1;i<num_psi;i++)
	        rho_r[i]=rho_r[i-1]+Count+1;
...
*/
			...
			if ((frePsi>=0) and (n%frePsi==0))
				if (Out_Mode==1)
					output_psi(n,rho_r[0],mirX,mirY,mirZ,mir,Solid);
				else
					output_psi_b(n,rho_r[0],mirX,mirY,mirZ,mir,Solid);
				
			if ((freDis>=0) and (n%freDis==0) and (Sub_Dis>0))	
			                {
			                        
			                Statistical_psi_general(rho_r,n,Solid); MPI_Barrier(MPI_COMM_WORLD);
					//Statistical_psi(rho_r[2],n,Solid);
					//==========for bodyforce output===========
...
//cout<<start_psi<<"     3333333333333     "<<wid_psi<<endl;
	
	for (int ci=1;ci<=Count;ci++)	
			
		{
			
				i=(int)(SupInv[ci]/((NY+1)*(NZ+1)))+disp[rank];
				j=(int)((SupInv[ci]%((NY+1)*(NZ+1)))/(NZ+1));
				m=(int)(SupInv[ci]%(NZ+1));   

			if ((i>=peak_n*wid_psi+start_psi) and (i<(peak_n+1)*wid_psi+start_psi))
			rho_r[ci]=1.0;
			else
			rho_r[ci]=0.0;
...
MPI_Barrier(MPI_COMM_WORLD);
	...
	if (rank==0)
		...
		MPI_Isend(sendr_s, Gcl[1], MPI_DOUBLE, rank+1, rank*2+1, MPI_COMM_WORLD,&request2[0]);
      		MPI_Isend(sendl_s, Gcr[mpi_size-1], MPI_DOUBLE, mpi_size-1, rank*2, MPI_COMM_WORLD,&request2[1]);
		MPI_Irecv(recvr_s, Gcr[0], MPI_DOUBLE, rank+1, (rank+1)*2, MPI_COMM_WORLD,&request2[2]);		
      		MPI_Irecv(recvl_s, Gcl[0], MPI_DOUBLE, mpi_size-1, (mpi_size-1)*2+1, MPI_COMM_WORLD,&request2[3] );
		...
		if (rank==mpi_size-1)
			{
			MPI_Isend(sendl_s, Gcr[rank-1], MPI_DOUBLE, rank-1, rank*2, MPI_COMM_WORLD,&request2[0]);
      			MPI_Isend(sendr_s, Gcl[0],  MPI_DOUBLE, 0, rank*2+1, MPI_COMM_WORLD,&request2[1]);
			MPI_Irecv(recvl_s, Gcl[rank], MPI_DOUBLE, rank-1, (rank-1)*2+1, MPI_COMM_WORLD,&request2[2] );
      			MPI_Irecv(recvr_s, Gcr[rank], MPI_DOUBLE, 0, 0, MPI_COMM_WORLD,&request2[3]);
			...
			else
			{
      			...
      			MPI_Isend(sendl_s, Gcr[rank-1], MPI_DOUBLE, rank-1, rank*2, MPI_COMM_WORLD,&request2[0]);
      			MPI_Isend(sendr_s, Gcl[rank+1], MPI_DOUBLE, rank+1, rank*2+1, MPI_COMM_WORLD,&request2[1]);
			MPI_Irecv(recvl_s, Gcl[rank], MPI_DOUBLE, rank-1, (rank-1)*2+1, MPI_COMM_WORLD,&request2[2]);
      			MPI_Irecv(recvr_s, Gcr[rank], MPI_DOUBLE, rank+1, (rank+1)*2, MPI_COMM_WORLD,&request2[3]);
...
void Statistical_psi_general(double** psi,int m, int*** Solid)
{
        int rank = MPI :: COMM_WORLD . Get_rank ();
	int mpi_size=MPI :: COMM_WORLD . Get_size ();
...
int ind;
	...
	if (rank==0)
		...
		for (int i=1;i<mpi_size;i++)
		 ...
		 rbuf_general=new double*[num_psi];
		         for (int i=0;i<num_psi;i++)
		                 rbuf_general[i]= new double[NX+1];
		         
		         ls_psi = new double[NX+1];
	        ...
	        if (Sub_Dis==2)
	                {
	                     ...
	                     for (int psi_n=0;psi_n<num_psi;psi_n++)
	                             {
	                             name.str("");
	                             name<<outputfile<<psi_n<<"_Statistical_data_concentration_X_"<<m<<".sta";
	                             ofstream out;
	                             out.open(name.str().c_str());
	                             for (int i=0;i<=NX;i++)
	                                     out<<rbuf_general[psi_n][i]<<endl;
	           ...
	           for (int psi_n=0;psi_n<num_psi;psi_n++)
	                   {
	                           ind=0;psi_total=0;
	                   for (int i=psi_n*wid_psi+start_psi;i<=NX;i++)
	                           {
	                           ls_psi[ind]=rbuf_general[psi_n][i];
	                           ind+=1;psi_total+=rbuf_general[psi_n][i];
	                           }
	                   for (int i=0;i<psi_n*wid_psi+start_psi;i++)
	                           {
	                           ls_psi[ind]=rbuf_general[psi_n][i];
	                           ind+=1;psi_total+=rbuf_general[psi_n][i];
	                           }
...
void Backup(int m,double* rho,double** psi, double** u, double** f, double*** g)
{
	...
	out.write((char *)(&psi[0][0]), sizeof(double)*num_psi*(Count+1));	
	...
	out.open(name5.str().c_str());
        ...
        out.write((char *)(&g[0][0][0]), sizeof(double)*num_psi*(Count+1)*7);    
	out.close();
