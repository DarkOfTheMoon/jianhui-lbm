...
double LatticeResolutionMicron = 1;		//Resolution of lattice in micrometres
...
void ReadVelocitiesBinSparse(char* FilePath){
	...
	const long long buffersize = 24*1048576;		//24MB
	...
	while(true){
		...
		if(x==NLattice_x){
			...
			if(y==NLattice_y){
				...
				z++;
				if(z==NLattice_z){
					goto ReadVelocitiesBinSparseReturnRJMP;
				}
			}
		}
	}
...
ReadVelocitiesBinSparseReturnRJMP:
	delete[] buf;
	fclose(InFile);
	return;
}
...
void ReadVelocitiesBin(char* FilePath){
	...
	while(true){
		...
		if(readpos == readl){
			...
			if(readl == 0){
				goto ReadVelocitiesBinReturnRJMP;
			}
			fread(buf, 1, readl, InFile);
			nRead += readl;
			readpos = 0;
	...
	while(true){
		...
		if(x==NLattice_x){
			...
			if(y==NLattice_y){
				...
				z++;
				if(z==NLattice_z){
					goto ReadVelocitiesSparseReturnRJMP;
				}
			}
		}
	}
...
ReadVelocitiesSparseReturnRJMP:
	delete[] buf;
	fclose(InFile);
	return;
}
