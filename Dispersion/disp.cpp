...
//Output folder
#define OutputFilesFolder "./"

//Output files
#define DispersionOutputFile "Dispersion_Coefficient_Portland.txt"		//File for dispersion coefficient
#define DistributionOutputFile "Particle_Distribution.txt"					//File for particle position distributions on lattice. Use %ST for simulation time, %RT for real scaled time
#define PropagatorOutputFile "Propagator_Distribution_Portland_t=%RT.txt"	//File for particle propagator distribution. Use %ST for simulation time, %RT for real scaled time
#define DecompositionLogFile "Decomposition_Log.txt"						//File for lattice decomposition log (MPI Mode 1)
	...
	while(i!=Filel){
		if(FileName[i]=='%' && i<(Filel-2)){
			if(FileName[i+1]=='S' && FileName[i+2]=='T'){			//Print simulation time
				...
				int n = snprintf(&Path[PathIndex],MaxPathLength-PathIndex,"%.0f",t);
				...
				int n = snprintf(&Path[PathIndex],MaxPathLength-PathIndex,"%.*f",ndp,tscaled);
...
AdvectDebugRestart:			//Jump here to restart advection with newly restored particle in debug mode
	...
	char DebugChar[128];
	int _n = snprintf(DebugChar,128,"%f%f%f",P->X,P->Y,P->Z);
	int _i=0;
	while(_i!=_n){
		if(DebugChar[_i]=='#'){
