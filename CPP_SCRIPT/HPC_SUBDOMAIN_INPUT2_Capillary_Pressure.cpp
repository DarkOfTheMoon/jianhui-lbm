...
double var1[total_number]={0.0001,0.00008,0.00006,0.00004,0.00002};
int time_interval_max=10000;

        
 	char poreFileName[128]="INPUT_CG_cap_";
	char poreFileName2[128]="LV60_cap_";
...
int time_int[total_number];
//========================================================
for (int i=0;i<total_number;i++)
	time_int[i]=(int)(time_interval_max*var1[i]/var1[0]);
...
out<<"1	        	      	:Pressure Or Velocity Boundary (1=YES, 0=No)"<<endl;
out<<"1				:Psi constant BC"<<endl;
out<<"0.0e-4 0.0 0.0	          	:body force for x,y,z"<<endl;
out<<"1 1.0 1 "<<1+var1[i]/6<<"	    		:Pressure Boundary in X direction ()p=c_s^2*/rho"<<endl;
out<<"0 1.0 0 1.0	    		:Pressure Boundary in Y direction ()"<<endl;
out<<"0 1.0 0 1.0	    	:Pressure Boundary in Z direction (Format detials can be found within this file)"<<endl;
...
out<<"/work/jy810/Sandpack/LV60/cap"<<i<<"_	:OUTPUT PATH,DEFAULT: ./ (INCLUDE / AT THE END)"<<endl;
out<<"2            :PRESSURE AND VELOCITY BOUNDARY CONDITION OPTIONS: 0,1,2,3: EBC_S,EBC_D,TOLKE_BC,NEBC_D"<<endl;
out<<"0  1000	:MULTI-COMPONENT STABALIZER: (a,b) a=0=OFF, a=1=ON, BODY FORCE APPLIED AFTER b steps"<<endl;
...
out<<"1 1			:0=OFF,1,2,3=ON,1=X,2=Y,3=Z;|| 1=PRESSURE,2=BODY FORCE"<<endl;
out<<time_int[i]*1<<" "<<1+var1[i]/6*2<<" 1.0 0.0		:ACT AT TIMESTEP n, PRESSURE N, PRE P, BODYFORCE(1)"<<endl;
out<<time_int[i]*2<<" "<<1+var1[i]/6*3<<" 1.0 0.0		:ACT AT TIMESTEP n, PRESSURE N, PRE P, BODYFORCE(2)"<<endl;
out<<time_int[i]*3<<" "<<1+var1[i]/6*4<<" 1.0 0.0		:ACT AT TIMESTEP n, PRESSURE N, PRE P, BODYFORCE(3)"<<endl;
out<<time_int[i]*4<<" "<<1+var1[i]/6*5<<" 1.0 0.0		:ACT AT TIMESTEP n, PRESSURE N, PRE P, BODYFORCE(4)"<<endl;
out<<time_int[i]*5<<" "<<1+var1[i]<<" 1.0 0.0		:ACT AT TIMESTEP n, PRESSURE N, PRE P, BODYFORCE(5)"<<endl;
...
out<<"#!/bin/sh "<<endl;
out<<"#PBS -N Sandpcap"<<i<<endl;
out<<"#PBS -l walltime=190:00:00 "<<endl;
//out<<"#PBS -l select=6:ncpus=12:icib=true "<<endl;
